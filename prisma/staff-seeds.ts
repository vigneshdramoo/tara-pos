export type StaffSeed = {
  name: string;
  username: string;
  email: string;
  role: "MANAGER" | "SALES_MANAGER" | "CASHIER";
  passwordEnvKey: string;
  defaultPassword: string;
};

export const staffSeeds: StaffSeed[] = [
  {
    name: "Daniel Jorge",
    username: "daniel",
    email: "daniel.jorge@tara.local",
    role: "MANAGER",
    passwordEnvKey: "SEED_MANAGER_PASSWORD",
    defaultPassword: "TARA2026",
  },
  {
    name: "Shireen Radley",
    username: "shireen",
    email: "shireen.radley@tara.local",
    role: "SALES_MANAGER",
    passwordEnvKey: "SEED_SALES_MANAGER_PASSWORD",
    defaultPassword: "TARA2026",
  },
  {
    name: "Cashier",
    username: "cashier",
    email: "cashier@tara.local",
    role: "CASHIER",
    passwordEnvKey: "SEED_CASHIER_PASSWORD",
    defaultPassword: "TARA2026",
  },
  {
    name: "Nur Syazwana Binti Armansah",
    username: "syaz",
    email: "nursyawana1@gmail.com",
    role: "CASHIER",
    passwordEnvKey: "SEED_SYAZ_PASSWORD",
    defaultPassword: "TARA2026",
  },
  {
    name: "Jermaine",
    username: "jermaine",
    email: "jermaine@tara.local",
    role: "CASHIER",
    passwordEnvKey: "SEED_JERMAINE_PASSWORD",
    defaultPassword: "TARA2026",
  },
  {
    name: "Sofea",
    username: "sofea",
    email: "sofea@tara.local",
    role: "CASHIER",
    passwordEnvKey: "SEED_SOFEA_PASSWORD",
    defaultPassword: "TARA2026",
  },
  {
    name: "Rielyna Iellyssya Anak Richard",
    username: "rielyna.richard",
    email: "rielyna.richard@tara.local",
    role: "CASHIER",
    passwordEnvKey: "SEED_RIELYNA_RICHARD_PASSWORD",
    defaultPassword: "TARA2026",
  },
];

export function getBootstrapPassword(seed: StaffSeed) {
  return process.env[seed.passwordEnvKey] ?? seed.defaultPassword;
}

export function hasConfiguredBootstrapPassword(seed: StaffSeed) {
  return Boolean(process.env[seed.passwordEnvKey]);
}
